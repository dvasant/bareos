...
if [[ ! -z "${GCSFUSE_BUCKET}" ]]; then
   ...
   gcsfuse --uid 101 --gid 101 -o allow_other --limit-bytes-per-sec "-1" --limit-ops-per-sec "-1" --stat-cache-ttl "1h" --type-cache-ttl "1h"  $GCSFUSE_BUCKET /var/lib/bareos/storage/
